...
namespace NanoTwitchLeafs.Controller
{
    public class NanoController
    {
        ...
        public NanoController(AppSettings appSettings)
        {
        ...
        public async Task<NanoLeafDevice> UpdateNanoLeafDevice(NanoLeafDevice nanoLeafDevice)
        {
            ...
            {
                if (device.DisplayName == nanoLeafDevice.DeviceName && device.IPAddress.ToString() != nanoLeafDevice.Address.ToString() + nanoLeafDevice.Port.ToString())
                {
                    var newAdress = device.IPAddress.Split(':');
                    nanoLeafDevice.Address = newAdress[0];
                    matchFound = true;
                }
            }
        ...
        public async Task<string> GetFirmwareVersionFromDevice(string nanoleafDeviceName)
        {
            ...
            {
                if (device.PublicName == nanoleafDeviceName)
                {
                    NanoleafControllerInfo controllerInfo = await GetControllerInfo(device);
        ...
        public async Task<string> GetFirmwareVersionFromDevice(IPAddress nanoleafDeviceAdress)
        {
            foreach (NanoLeafDevice device in _appSettings.NanoSettings.NanoLeafDevices)
            {
                if (device.Address == nanoleafDeviceAdress.ToString())
                {
                    NanoleafControllerInfo controllerInfo = await GetControllerInfo(device);
                    return controllerInfo.firmwareVersion.ToString();
                }
        ...
        public async Task IdentifyController(NanoLeafDevice nanoLeafDevice)
        {
            using (var client = new HttpClient())
            {
                client.Timeout = TimeSpan.FromSeconds(5);
                client.DefaultRequestHeaders.ExpectContinue = false;
                _logger.Debug($"Identify Controller with Adress {nanoLeafDevice.Address}");
        ...
        public async Task<bool> UpdateAllNanoleafDevices()
        {
            ...
            foreach (var device in _appSettings.NanoSettings.NanoLeafDevices)
            {
                ...
                _logger.Info($"Updated Adress from Device {device.PublicName} from {device.Address} to {updatedDevice.Address}");
                count++;
            }
            ...
            {
                _appSettings.NanoSettings.NanoLeafDevices.Clear();
                _appSettings.NanoSettings.NanoLeafDevices = updatedNanoLeafDevices;
